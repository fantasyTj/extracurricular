...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        def temp_func(z_i, z_j): return torch.exp(sim(z_i, z_j)/tau)
        ##############################################################################
        # TODO: Start of your code.                                                  #
        #                                                                            #
        ...
        l_numer = temp_func(z_k, z_k_N)

        l_denom1, l_denom2 = 0, 0
        for i in range(2*N):
            if i != k:
                l_denom1 += temp_func(out[i], z_k)
            if i != k+N:
                l_denom2 += temp_func(out[i], z_k_N)

        total_loss += -(torch.log(l_numer/l_denom1) +
                        torch.log(l_numer/l_denom2))
...
def sim_positive_pairs(out_left, out_right):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N = out_left.shape[0]
    pos_pairs = torch.matmul(out_left, torch.transpose(out_right, 0, 1))
    mask = torch.eye(N).bool()
    pos_pairs = pos_pairs.masked_select(mask).view(N, -1)
    pos_pairs /= torch.linalg.norm(
        out_left, dim=1, keepdim=True) * torch.linalg.norm(out_right, dim=1, keepdim=True)
...
def compute_sim_matrix(out):
    ...
    sim_matrix = torch.matmul(out, torch.transpose(out, 0, 1))
    norm_vec = torch.linalg.norm(out, dim=1, keepdim=True)
    sim_matrix /= norm_vec
    sim_matrix /= norm_vec.view(1, -1)
