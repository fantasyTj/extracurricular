...
def missing_digits(n):
    ...
    if n // 10 == 0:
        ...
        draw = n % 100
        delta = draw % 10 - draw // 10
        return missing_digits(n // 10) + (delta - 1 if delta > 1 else 0)
...
def count_change(total):
    ...
    def helper(total, bound):
        if total == 0:
            ...
            return helper(total - bound, bound) + helper(total, bound // 2)


    return helper(total, 2**(floor(log2(total))))
    ...
    if n == 1:
        ...
        for i in range(1, 4):
            if i != start and i != end:
                avai = i
        # move_stack(1, start, avai)
        # move_stack(n - 1, start, end)
        # move_stack(1, avai, end)
        move_stack(n - 1, start, avai)
        move_stack(1, start, end)
        move_stack(n - 1, avai, end)
...
def make_anonymous_factorial():
    ...
    return (lambda f: (lambda n: 1 if n == 1 else n * f(n-1, f)))(lambda n, f: 1 if n == 1 else n * f(n-1, f))
