...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    invalid_pwd = []
    def withdraw(amount, attempt_pwd):
        if len(invalid_pwd) >= 3:
            return 'Too many incorrect attempts. Attempts: ' + str(invalid_pwd)
        elif attempt_pwd != password:
            invalid_pwd.append(attempt_pwd)
...
def repeated(t, k):
    ...
    assert k > 1
    "*** YOUR CODE HERE ***"
    repeated_num, repeated_count = next(t), 1
    while True:
        cur = next(t)
        if cur == repeated_num:
            repeated_count += 1
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    """
    "*** YOUR CODE HERE ***"
    def withdraw_wrapper(amount, attempt_pwd):
        if attempt_pwd == new_pass:
            attempt_pwd = old_pass
        return withdraw(amount, attempt_pwd)
