...
mconf = md.GPTConfig(pretrain_dataset.vocab_size, pretrain_dataset.block_size,
...
if args.function == 'pretrain':
    ...
    finetuning_dataset = dataset.NameDataset(pretrain_dataset, text)

    if args.reading_params_path == None:
        tconf = trner.TrainerConfig(max_epochs=75, batch_size=256, learning_rate=args.finetune_lr,
             lr_decay=True, warmup_tokens=512*20, final_tokens=200*len(pretrain_dataset)*block_size,
             num_workers=4, writer=writer)
        trainer = trner.Trainer(model, finetuning_dataset, None, tconf)
        trainer.train()
