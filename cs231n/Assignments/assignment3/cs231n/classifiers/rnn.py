...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
            h, cache = rnn_forward(embed, h0, Wx, Wh, b)
        else:
            h, cache = lstm_forward(embed, h0, Wx, Wh, b)
        ...
        grads['W_proj'], grads['b_proj'] = np.dot(
            features.T, dh0), np.sum(dh0, axis=0)
    ...
    def sample(self, features, max_length=30):
        ...
        # print(N)
        cur_word = self._start * np.ones((N,))
        captions[:, 0] = cur_word
        # cur_word = np.max(cur_scores, axis=1, keepdims=True)
        # captions[:, 1] = cur_word

        for i in range(1, max_length):
            next_h, _ = rnn_step_forward(
                W_embed[cur_word.astype('int')], next_h, Wx, Wh, b)
            # print(next_h.shape)
            cur_scores = np.dot(next_h, W_vocab) + b_vocab
            cur_word = np.argmax(cur_scores, axis=1)
            # print(cur_scores.shape, cur_word.shape)
            captions[:, i] = cur_word
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ############################################################################
        #                             END OF YOUR CODE                             #
