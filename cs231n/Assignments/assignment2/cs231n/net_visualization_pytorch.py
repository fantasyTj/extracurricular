...
def compute_saliency_maps(X: torch.Tensor, y: torch.Tensor, model: torch.nn.Module):
    """
    ...
    grads = X.grad
    saliency, _ = torch.max(torch.abs(grads), dim=1)
...
def make_fooling_image(X: torch.Tensor, target_y, model: torch.nn.Module):
    """
    ...
    while (True):
        ...
        # X_fooling.detach_()
        X_fooling.data += learning_rate * grads
        # X_fooling = X_fooling.requires_grad_()
