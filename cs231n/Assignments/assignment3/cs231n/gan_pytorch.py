...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss = bce_loss(logits_real, torch.ones_like(
        logits_real).type(dtype).squeeze())
    loss += bce_loss(logits_fake,
                     torch.zeros_like(logits_fake).type(dtype).squeeze())
...
def generator_loss(logits_fake):
    ...
    labels = torch.ones_like(logits_fake).type(dtype)
    labels.squeeze_()
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    def square_loss(x, label):
        temp = (x-label).type(dtype)
        dim = temp.shape[0]
        return torch.sum(temp*temp) / dim

    loss = square_loss(scores_real, 1) / 2
    loss += square_loss(scores_fake, 0) / 2
...
def ls_generator_loss(scores_fake):
    ...
    loss = square_loss(scores_fake, 1) / 2
