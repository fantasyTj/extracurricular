...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    tanh, prevh, x, Wh, Wx = cache
    dtanh = dnext_h*(1-tanh*tanh)
    db = np.sum(dtanh, axis=0)

    dprev_h = np.dot(dtanh, Wh.T)
    dx = np.dot(dtanh, Wx.T)
    dWx = np.dot(x.T, dtanh)
    dWh = np.dot(prevh.T, dtanh)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    # tanh, prevh, x, Wh, Wx
    ts_x = np.transpose(x, (1, 0, 2))
    prevh = h0
    h_out = None
    T = ts_x.shape[0]
    ts_h = np.zeros((x.shape[1], x.shape[0], h0.shape[1]))
    ts_tanh = np.zeros_like(ts_h)

    for i in range(T):
        h_out, cache = rnn_step_forward(ts_x[i], prevh, Wx, Wh, b)
        # X = np.dot(ts_x[i], Wx)
        # H = np.dot(h_in, Wh)
        # h_out = np.tanh(X + H + b)
        ts_h[i] = prevh
    ...
    h = np.transpose(ts_tanh, (1, 0, 2))
    cache = (ts_tanh, ts_h, ts_x, Wh, Wx)
...
def rnn_backward(dh, cache):
    ...
    ts_tanh, ts_prevh, ts_x, Wh, Wx = cache
    ts_dh = np.transpose(dh, (1, 0, 2))
    T, N, H, D = ts_dh.shape[0], ts_dh.shape[1], ts_dh.shape[2], ts_x.shape[-1]
    ts_dx = np.zeros((T, N, D))
    dWh = np.zeros((H, H))
    dWx = np.zeros((D, H))
    db = np.zeros((H,))
    dprevh = np.zeros((N, H))
    ...
    for i in range(1, T+1):
        dx_, dprevh, dWx_, dWh_, db_ = rnn_step_backward(
            ts_dh[-i]+dprevh, (ts_tanh[-i], ts_prevh[-i], ts_x[-i], Wh, Wx))
