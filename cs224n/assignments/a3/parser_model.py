...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(
            torch.tensor(np.zeros((self.n_features*embeddings.shape[1], self.hidden_size)), dtype=torch.float))
        nn.init.xavier_normal_(self.embed_to_hidden_weight)
        self.embed_to_hidden_bias = nn.Parameter(
            torch.tensor(np.zeros(self.hidden_size), dtype=torch.float))
        nn.init.uniform_(self.embed_to_hidden_bias)

        self.dropout = nn.Dropout(self.dropout_prob)

        self.hidden_to_logits_weight = nn.Parameter(
            torch.tensor(np.zeros((self.hidden_size, self.n_classes)), dtype=torch.float))
        nn.init.xavier_normal_(self.hidden_to_logits_weight)
        self.hidden_to_logits_bias = nn.Parameter(
            torch.tensor(np.zeros(self.n_classes), dtype=torch.float))
        nn.init.normal_(self.hidden_to_logits_bias)
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.Tensor.view(torch.index_select(
            self.embeddings, 0, torch.flatten(w)), (w.shape[0], -1))
        # END YOUR CODE
        return x
    ...
    def forward(self, w):
        ...
        h = F.relu(torch.matmul(self.embedding_lookup(w), self.embed_to_hidden_weight) +
