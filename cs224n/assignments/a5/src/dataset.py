...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        chuck = self.data[idx]
        trunc_pos = random.randint(4, int(self.block_size*7/8))
        chuck = chuck[:trunc_pos]
        trunc_len = len(chuck)
        masked_len = int(random.random()*(0.5*trunc_len))
        start_pos_masked = random.randint(1, trunc_len-masked_len-1)
        start_pos_suffix = start_pos_masked + masked_len
        prefix, masked_content, suffix = chuck[:start_pos_masked], chuck[start_pos_masked: start_pos_suffix], chuck[start_pos_suffix: ]
        ...
        x = torch.tensor([self.stoi[i] for i in masked_string[:-1]], dtype=torch.long)
        y = torch.tensor([self.stoi[i] for i in masked_string[1: ]], dtype=torch.long)
        return x,y
