...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        batch = unfinished_parses[:batch_size] if len(
            unfinished_parses) > batch_size else unfinished_parses
        transition_list = model.predict(batch)
        for trans_i in range(len(transition_list)):
            batch[trans_i].parse_step(transition_list[trans_i])
        for pp in batch:
            if len(pp.buffer) == 0 and len(pp.stack) == 1:
                unfinished_parses.remove(pp)
                dependencies[partial_parses.index(pp)] = pp.dependencies
