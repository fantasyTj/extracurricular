...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(
            input_size=embed_size, hidden_size=hidden_size, num_layers=1, bidirectional=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X = torch.permute(self.post_embed_cnn(
            torch.permute(X, (1, 2, 0))), (2, 0, 1))
        # print("X shape: ", X.size())

        X = pack_padded_sequence(X, source_lengths)
        # print("pack X shape: ", X.data.size())
        enc_hiddens, (hn, cn) = self.encoder(X)
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens)
        enc_hiddens = torch.permute(enc_hiddens, (1, 0, 2))
        # print("enc shape: ", enc_hiddens.size())
        # print("hn shape: ", hn.size())

        dec_init_state = (self.h_projection(torch.cat(
            (hn[0], hn[1]), dim=-1)), self.c_projection(torch.cat((cn[0], cn[1]), dim=-1)))
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        a_t = torch.squeeze(torch.bmm(torch.unsqueeze(
            alpha_t, dim=1), enc_hiddens), dim=1)
