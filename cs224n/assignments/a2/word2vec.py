...
def naiveSoftmaxLossAndGradient(
    ...
    # to integer overflow.
    dot_product = np.dot(outsideVectors, centerWordVec)
    sm = softmax(dot_product)
    loss = -np.log(sm[outsideWordIdx])

    gradCenterVec = -outsideVectors[outsideWordIdx] + \
        np.sum(np.reshape(sm, (-1, 1)) * outsideVectors, axis=0)
    gradOutsideVecs = np.reshape(sm, (-1, 1)) * centerWordVec
...
def negSamplingLossAndGradient(
    ...
    sampledVecs = -outsideVectors[indices]
    sampledVecs[0] = -sampledVecs[0]
    dotProduct = np.dot(sampledVecs, centerWordVec)
    sigmoidVecs = np.reshape(sigmoid(dotProduct), (-1, 1))
    loss = -np.sum(np.log(sigmoidVecs))
    gradCenterVec = np.sum((sigmoidVecs-1)*sampledVecs, 0)
    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradTemp = (1-sigmoidVecs) * centerWordVec
    gradTemp[0] = -gradTemp[0]
    np.add.at(gradOutsideVecs, indices, gradTemp)  # !!!
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for i in range(len(outsideWords)):
        tempLoss, tempCenter, tempOutside = word2vecLossAndGradient(
            centerWordVec, word2Ind[outsideWords[i]], outsideVectors, dataset)
        loss += tempLoss
        gradCenterVecs[centerWordInd] += tempCenter
        gradOutsideVectors += tempOutside
    # END YOUR CODE
